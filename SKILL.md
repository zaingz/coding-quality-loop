...
The advisor topology needs a strong executor (Sonnet-class or better). Cognition's production finding: with a weak primary the quality ceiling is set by the executor and the pattern underperforms — with a GLM-class implementer, keep orchestrator-delegates instead of defaulting to advisor. Treat this as a condition on the default, not a reason to skip the consult when the executor is strong.
