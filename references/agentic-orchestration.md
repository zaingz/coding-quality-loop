...
| Phase | Step | Profile | Model class | Required artifact | Gate |
|---|---|---|---|---|---|
...
- **Smart friend (optional consult).** The implementer can consult a stronger model on defined
  triggers — 2 failed repair attempts, merge conflicts, or architecture uncertainty. The
  stronger model gets a fork of the implementer's context and responds with *guidance, not
  code*. Cognition's finding: this works best when both models are strong; making it work with
  an asymmetrically weaker primary is still an open problem. Cross-frontier delegation is a
  **capability router** — route to whichever model is best at the sub-task, not just to a
  "smarter" one.
