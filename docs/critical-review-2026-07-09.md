...
- **Methodology contradiction:** `examples/rust-procmon-eval-2026-07-03/README.md:85` states its arms were built by Perplexity model-proxy subagents, "same pattern as the existing sudoku-agent-eval-2026-07-01" — yet the README headlines Sudoku 07-01 as one of "three live cross-agent evals" and its results.json records no CLI versions or invocation method. Only webapp 07-07 is unambiguously real-CLI (exact versions, browser-verified). This is the single most quotable weakness if the repo hits a skeptical audience.
