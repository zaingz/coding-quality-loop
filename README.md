...
- **Skills Hub publish checklist.** Before publishing to the
  ...
  3. Run `python3 scripts/quality_loop.py check-config assets/quality-loop.config.example.json`
     and the full eval suite (all suites green: 19 static + 62 behavioral + 32 memory + 51 reality + 30 routing + 56 hook = 250 gate cases, plus 40 add-on cases for the control plane).
